...
function guideClick(){
	$("#overlay").overlay({api:true,left:100,top:200}).load();
}
...
function validateForm(){
	...
	if($('#artistname').val() == artistStart){
		submit = false;
		$('#artist-error').html("*Please enter the artists's name");
	}
	if($('#title').val() == titleStart){
		submit = false;
		$('#title-error').html("*Please enter a title");
	}
	...
	if($('#firstname').val() == nameStart){
		submit = false;
		$('#name-error').html("*Please enter you first name");
	}
	return submit;	
}
